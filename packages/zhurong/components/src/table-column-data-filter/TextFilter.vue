<template>
  <div v-if="currOption" class="my-filter-input">
    <VxeInput v-model="currOption.data" mode="text" placeholder="支持回车筛选"
               @input="changeOptionEvent"
               @keyup="keyupEvent"></VxeInput>
  </div>
</template>
...
import type {VxeGlobalRendererHandles, VxeTableDefines} from '@vben/plugins/vxe-table';
